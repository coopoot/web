...
Traccar.app.getAttributePreference('volumeUnit'))},volumeConverter:function(value){return Ext.getStore('VolumeUnits').convertValue(value,Traccar.app.getAttributePreference('volumeUnit'))},durationFormatter:function(value){return Math.floor(value/36E5)+' '+Strings.sharedHourAbbreviation+' '+Math.floor(value%36E5/6E4)+' '+Strings.sharedMinuteAbbreviation},deviceIdFormatter:function(value){return Ext.getStore('Devices').getById(value).get('name')},groupIdFormatter:function(value){var group;return 0!==
value?(group=Ext.getStore('AllGroups'),0===group.getTotalCount()&&(group=Ext.getStore('Groups')),(group=group.getById(value))?group.get('name'):value):null},geofenceIdFormatter:function(value){var store;return 0!==value?(store=Ext.getStore('AllGeofences'),0===store.getTotalCount()&&(store=Ext.getStore('Geofences')),(value=store.getById(value))?value.get('name'):''):null},driverUniqueIdFormatter:function(value){var driver;return 0!==value?(driver=Ext.getStore('AllDrivers'),0===driver.getTotalCount()&&
(driver=Ext.getStore('Drivers')),(driver=driver.findRecord('uniqueId',value,0,!1,!0,!0))?value+' ('+driver.get('name')+')':value):null},lastUpdateFormatter:function(value){var interval;if(value){value=Math.floor((new Date-value)/1E3);0>value&&(value=0);interval=Math.floor(value/86400);if(1<interval)return interval+' '+Strings.sharedDays;interval=Math.floor(value/3600);return 1<interval?interval+' '+Strings.sharedHours:Math.floor(value/60)+' '+Strings.sharedMinutes}return null},commandTypeFormatter:function(value){var name=
Strings['command'+value.charAt(0).toUpperCase()+value.slice(1)];return name?name:value},defaultFormatter:function(value){return'number'===typeof value?Number(value.toFixed(Traccar.Style.numberPrecision)):'boolean'===typeof value?value?Ext.Msg.buttonText.yes:Ext.Msg.buttonText.no:value instanceof Date?Traccar.app.getPreference('twelveHourFormat',!1)?Ext.Date.format(value,Traccar.Style.dateTimeFormat12):Ext.Date.format(value,Traccar.Style.dateTimeFormat24):value},getFormatter:function(key){var self=
this;switch(key){case 'latitude':case 'longitude':return function(value){return self.coordinateFormatter(key,value)};case 'speed':return this.speedFormatter;case 'course':return this.courseFormatter;case 'distance':case 'accuracy':return this.distanceFormatter;case 'duration':return this.durationFormatter;case 'deviceId':return this.deviceIdFormatter;case 'groupId':return this.groupIdFormatter;case 'geofenceId':return this.geofenceIdFormatter;case 'lastUpdate':return this.lastUpdateFormatter;case 'spentFuel':return this.volumeFormatter;
...
{name:'coordinateFormat',type:'string'},{name:'attributes'}],proxy:{type:'ajax',url:'api/server',actionMethods:{update:'PUT'},writer:{type:'json',writeAllFields:!0}}});
Ext.define('Traccar.model.User',{extend:Ext.data.Model,identifier:'negative',fields:[{name:'id',type:'int'},{name:'name',type:'string'},{name:'login',type:'string'},{name:'email',type:'string'},{name:'password',type:'string'},{name:'phone',type:'string'},{name:'readonly',type:'boolean'},{name:'admin',type:'boolean'},{name:'map',type:'string'},{name:'latitude',type:'float'},{name:'longitude',type:'float'},{name:'zoom',type:'int'},{name:'twelveHourFormat',type:'boolean'},{name:'coordinateFormat',type:'string'},
{name:'disabled',type:'boolean'},{name:'expirationTime',type:'date',dateFormat:'c'},{name:'deviceLimit',type:'int'},{name:'userLimit',type:'int'},{name:'deviceReadonly',type:'boolean'},{name:'limitCommands',type:'boolean'},{name:'token',type:'string'},{name:'attributes'}],proxy:{type:'rest',url:'api/users',writer:{type:'json',writeAllFields:!0}}});
...
Ext.define('Traccar.model.Notification',{extend:Ext.data.Model,identifier:'negative',fields:[{name:'id',type:'int'},{name:'type',type:'string'},{name:'always',type:'bool'},{name:'attributes'},{name:'web',type:'bool'},{name:'mail',type:'bool'},{name:'sms',type:'bool'}]});
...
Ext.define('Traccar.model.KnownNotification',{extend:Ext.data.Model,idProperty:'type',fields:[{name:'type',type:'string'},{name:'name',convert:function(v,rec){return Traccar.app.getEventString(rec.get('type'))},depends:['type']}]});Ext.define('Traccar.store.Groups',{extend:Ext.data.Store,model:'Traccar.model.Group',proxy:{type:'rest',url:'api/groups',writer:{writeAllFields:!0}}});
...
key;case 'dms':return value=Math.abs(value),unit=Math.floor(value),minutes=Math.floor(60*(value-unit)),value=Math.round(3600*(value-unit-minutes/60)),unit+'° '+minutes+"' "+value+'" '+key;default:return value.toFixed(Traccar.Style.coordinatePrecision)+'°'}}});Ext.define('Traccar.store.CommandTypes',{extend:Ext.data.Store,model:'Traccar.model.KnownCommand',proxy:{type:'rest',url:'api/commands/types',listeners:{exception:function(proxy,response){Traccar.app.showError(response)}}}});
...
combobox.allowDecimals=!0;combobox.dataType=newValue.get('dataType');combobox.maxValue=newValue.get('maxValue');combobox.minValue=newValue.get('minValue');break;case 'boolean':combobox.xtype='checkboxfield';combobox.inputValue=!0;combobox.uncheckedValue=!1;break;case 'color':combobox.xtype='customcolorpicker';break;default:if(newValue.get('dataType'))switch(combobox.xtype='combobox',combobox.queryMode='local',combobox.editable=!1,newValue.get('dataType')){case 'distanceUnit':combobox.store='DistanceUnits';
combobox.displayField='name';combobox.valueField='key';break;case 'speedUnit':combobox.store='SpeedUnits';combobox.displayField='name';combobox.valueField='key';break;case 'volumeUnit':combobox.store='VolumeUnits';combobox.displayField='fullName';combobox.valueField='key';break;case 'timezone':combobox.store='AllTimezones',combobox.displayField='key'}else combobox.xtype='textfield'}valueField.getXType()!==combobox.xtype?(this.getView().down('form').insert(this.getView().down('form').items.indexOf(valueField),
...
(config.listeners={}),config.listeners.afterrender=function(){this.units||(this.units=Ext.create({xtype:'combobox',renderTo:'numberUnitEl',store:'TimeUnits',displayField:'name',valueField:'key',editable:!1,numberField:this,value:'s',width:'70px',listeners:{select:function(){this.numberField.step=this.getStore().convertValue(1,this.getValue(),!0)}}}))}}config.afterBodyEl=this.unitEl+unitName+'\x3c/div\x3e\x3c/div\x3e';config.rawToValue=function(rawValue){return this.units?this.units.getStore().convertValue(rawValue,
this.units.getValue(),!0):this.parseValue(rawValue)};config.valueToRaw=function(value){return this.units?this.units.getStore().convertValue(value,this.units.getValue(),!1):this.parseValue(value)};config.units&&(config.step=config.units.getStore().convertValue(1,config.units.getValue(),!0))}this.callParent(arguments)}});
...
Ext.define('Traccar.view.edit.Attributes',{extend:Traccar.view.GridPanel,xtype:'attributesView',controller:'attributes',tbar:{xtype:'editToolbar'},listeners:{selectionchange:'onSelectionChange'},columns:{defaults:{flex:1,minWidth:Traccar.Style.columnWidthNormal},items:[{text:Strings.sharedName,dataIndex:'name',filter:'string',renderer:function(value){var attribute;this.attributesStore&&(attribute=Ext.getStore(this.attributesStore).getById(value));return attribute&&attribute.get('name')?attribute.get('name'):
value}},{text:Strings.stateValue,dataIndex:'value',renderer:function(value,metaData,record){var attribute;this.attributesStore&&(attribute=Ext.getStore(this.attributesStore).getById(record.get('name')));return attribute&&'speed'===attribute.get('dataType')?Ext.getStore('SpeedUnits').formatValue(value,Traccar.app.getAttributePreference('speedUnit','kn'),!0):attribute&&'distance'===attribute.get('dataType')?Ext.getStore('DistanceUnits').formatValue(value,Traccar.app.getAttributePreference('distanceUnit',
'km'),!0):value}}]}});
...
queryMode:'local',displayField:'name',valueField:'id',editable:!1},{xtype:'textfield',name:'phone',fieldLabel:Strings.sharedPhone},{xtype:'textfield',name:'model',fieldLabel:Strings.deviceModel},{xtype:'textfield',name:'contact',fieldLabel:Strings.deviceContact},{xtype:'combobox',name:'category',fieldLabel:Strings.deviceCategory,store:'DeviceImages',queryMode:'local',displayField:'name',valueField:'key',editable:!1,listConfig:{getInnerTpl:function(){return'\x3ctable\x3e\x3ctr valign\x3d"middle" \x3e\x3ctd\x3e\x3cdiv align\x3d"center" style\x3d"width:40px;height:40px;" \x3e{[new XMLSerializer().serializeToString(Traccar.DeviceImages.getImageSvg(Traccar.Style.mapColorOnline, false, 0, values.key))]}\x3c/div\x3e\x3c/td\x3e\x3ctd\x3e{name}\x3c/td\x3e\x3c/tr\x3e\x3c/table\x3e'}}}]}]}});
Ext.define('Traccar.view.permissions.Geofences',{extend:Traccar.view.permissions.Base,xtype:'linkGeofencesView',columns:{items:[{text:Strings.sharedName,dataIndex:'name',flex:1,minWidth:Traccar.Style.columnWidthNormal,filter:'string'}]}});
...
Traccar.app.getPreference('deviceReadonly',!1)&&!Traccar.app.getUser().get('admin');readonly=Traccar.app.getPreference('readonly',!1)&&!Traccar.app.getUser().get('admin');this.lookupReference('toolbarAddButton').setDisabled(readonly||deviceReadonly);this.lookupReference('toolbarDeviceMenu').setHidden(readonly||deviceReadonly);setInterval(function(){self.getView().getView().refresh()},Traccar.Style.refreshPeriod)},onCommandClick:function(){var deviceId,dialog,typesStore;deviceId=this.getView().getSelectionModel().getSelection()[0].get('id');
dialog=Ext.create('Traccar.view.dialog.SendCommand');dialog.deviceId=deviceId;typesStore=dialog.lookupReference('commandsComboBox').getStore();typesStore.getProxy().setExtraParam('deviceId',deviceId);Traccar.app.getPreference('limitCommands',!1)||typesStore.add({id:0,description:Strings.sharedNew});typesStore.load({addRecords:!0});typesStore=dialog.lookupReference('commandType').getStore();typesStore.getProxy().setExtraParam('deviceId',deviceId);typesStore.load();dialog.show()},updateButtons:function(selected){var readonly,
deviceReadonly,empty;deviceReadonly=Traccar.app.getPreference('deviceReadonly',!1)&&!Traccar.app.getUser().get('admin');readonly=Traccar.app.getPreference('readonly',!1)&&!Traccar.app.getUser().get('admin');empty=0===selected.length;this.lookupReference('toolbarEditButton').setDisabled(empty||readonly||deviceReadonly);this.lookupReference('toolbarRemoveButton').setDisabled(empty||readonly||deviceReadonly);deviceReadonly=this.lookupReference('toolbarDeviceMenu');deviceReadonly.device=empty?null:selected[0];
deviceReadonly.setDisabled(empty);this.lookupReference('deviceCommandButton').setDisabled(empty||readonly)},onSelectionChange:function(selection,selected){this.updateButtons(selected);0<selected.length?this.fireEvent('selectdevice',selected[0],!0):this.fireEvent('deselectfeature')},selectDevice:function(device){this.getView().getSelectionModel().select([device],!1,!0);this.updateButtons(this.getView().getSelectionModel().getSelected().items);this.getView().getView().focusRow(device)},selectReport:function(position){void 0!==
position&&this.deselectFeature()},onUpdateDevice:function(){this.updateButtons(this.getView().getSelectionModel().getSelected().items)},deselectFeature:function(){this.getView().getSelectionModel().deselectAll()}});
...
reference:'menuNotificationsButton'},{text:Strings.sharedComputedAttributes,glyph:'xf0ae@FontAwesome',handler:'onComputedAttributesClick',reference:'menuComputedAttributesButton'},{text:Strings.sharedSavedCommands,glyph:'xf093@FontAwesome',handler:'onCommandsClick',reference:'menuCommandsButton'},{hidden:!0,text:Strings.sharedDeviceDistance,glyph:'xf0e4@FontAwesome',handler:'onDeviceDistanceClick',reference:'menuDeviceDistanceButton'}]}});
...
reference:'toolbarEditButton',glyph:'xf040@FontAwesome',tooltip:Strings.sharedEdit},{handler:'onRemoveClick',reference:'toolbarRemoveButton',glyph:'xf00d@FontAwesome',tooltip:Strings.sharedRemove},{handler:'onCommandClick',reference:'deviceCommandButton',glyph:'xf093@FontAwesome',tooltip:Strings.deviceCommand},{xtype:'deviceMenu',reference:'toolbarDeviceMenu',enableToggle:!1}]},listeners:{selectionchange:'onSelectionChange'},viewConfig:{enableTextSelection:!0,getRowClass:function(record){return(record=
...
Ext.define('Traccar.view.StateController',{extend:Ext.app.ViewController,alias:'controller.state',config:{listen:{controller:{'*':{selectdevice:'selectDevice',selectreport:'selectReport',deselectfeature:'deselectFeature'}},global:{stategeocode:'onGeocode'},store:{'#LatestPositions':{add:'updateLatest',update:'updateLatest'},'#ReportRoute':{clear:'clearReport'}}}},init:function(){var i,attributesList;(Traccar.app.getUser().get('admin')||!Traccar.app.getUser().get('deviceReadonly')&&!Traccar.app.getPreference('readonly',
!1))&&this.lookupReference('computedAttributesButton').setDisabled(Traccar.app.getBooleanAttributePreference('ui.disableComputedAttributes'));i=Traccar.app.getAttributePreference('ui.hidePositionAttributes');this.hideAttributes={};if(i)for(attributesList=i.split(/[ ,]+/).filter(Boolean),i=0;i<attributesList.length;i++)this.hideAttributes[attributesList[i]]=!0},onComputedAttributesClick:function(){Ext.create('Traccar.view.BaseWindow',{title:Strings.sharedComputedAttributes,items:{xtype:'computedAttributesView'}}).show()},
keys:function(){var i,list,result;result={};list='fixTime latitude longitude valid accuracy altitude speed course address protocol'.split(' ');for(i=0;i<list.length;i++)result[list[i]]={priority:i,name:Strings['position'+list[i].replace(/^\w/g,function(s){return s.toUpperCase()})]};return result}(),updateLatest:function(store,data){Ext.isArray(data)||(data=[data]);for(store=0;store<data.length;store++)this.deviceId===data[store].get('deviceId')&&(this.position=data[store],this.updatePosition())},
formatValue:function(value){return'number'===typeof id?Number(value.toFixed(2)):value},findAttribute:function(record){return record.get('deviceId')===this.position.get('deviceId')&&record.get('attribute')===this.lookupAttribute},updatePosition:function(){var attributes,store,key,name,value;store=Ext.getStore('Attributes');store.removeAll();for(key in this.position.data)this.position.data.hasOwnProperty(key)&&void 0!==this.keys[key]&&store.add(Ext.create('Traccar.model.Attribute',{priority:this.keys[key].priority,
name:this.keys[key].name,value:Traccar.AttributeFormatter.getFormatter(key)(this.position.get(key))}));attributes=this.position.get('attributes');if(attributes instanceof Object)for(key in attributes)attributes.hasOwnProperty(key)&&!this.hideAttributes[key]&&(this.lookupAttribute=key,name=Ext.getStore('PositionAttributes').getAttributeName(key,!0),value=void 0!==this.position.get('attribute.'+key)?Traccar.AttributeFormatter.getAttributeFormatter(key)(this.position.get('attribute.'+key)):Traccar.AttributeFormatter.defaultFormatter(attributes[key]),
store.add(Ext.create('Traccar.model.Attribute',{priority:1024,name:name,attribute:key,value:value})))},selectDevice:function(device){this.deviceId=device.get('id');(device=Ext.getStore('LatestPositions').findRecord('deviceId',this.deviceId,0,!1,!1,!0))?(this.position=device,this.updatePosition()):(this.position=null,Ext.getStore('Attributes').removeAll())},selectReport:function(position){position instanceof Traccar.model.Position&&(this.deviceId=null,this.position=position,this.updatePosition())},
deselectFeature:function(){this.position=this.deviceId=null;Ext.getStore('Attributes').removeAll()},clearReport:function(){this.deviceId||(this.position=null,Ext.getStore('Attributes').removeAll())},onGeocode:function(){var positionId=this.position.getId();this.position.get('address')||Ext.Ajax.request({scope:this,method:'GET',url:'api/server/geocode',params:{latitude:this.position.get('latitude'),longitude:this.position.get('longitude')},success:function(response){this.position&&this.position.getId()===
positionId&&(this.position.set('address',response.responseText),this.updatePosition())},failure:function(response){Traccar.app.showError(response)}})}});
...
callingPanel.toTime=this.lookupReference('toTimeField').getValue();callingPanel.period=this.lookupReference('periodField').getValue();callingPanel.updateButtons();button.up('window').close()},onPeriodChange:function(combobox,newValue){var to;combobox='custom'===newValue;this.lookupReference('fromContainer').setHidden(!combobox);this.lookupReference('toContainer').setHidden(!combobox);if(!combobox){combobox=new Date;to=new Date;switch(newValue){case 'today':to.setDate(to.getDate()+1);break;case 'yesterday':combobox.setDate(to.getDate()-
...
void 0!==this.period&&dialog.lookupReference('periodField').setValue(this.period);dialog.show()},updateButtons:function(){var reportType,disabled,time;reportType=this.lookupReference('reportTypeField').getValue();disabled=this.deviceId&&0!==this.deviceId.length||this.groupId&&0!==this.groupId.length;time=this.fromDate&&this.fromTime&&this.toDate&&this.toTime;disabled=!reportType||!disabled||!time;this.lookupReference('showButton').setDisabled(disabled);this.lookupReference('exportButton').setDisabled('chart'===
reportType||disabled)},onReportClick:function(button){var reportType,from,to;this.getGrid().getSelectionModel().deselectAll();(reportType=this.lookupReference('reportTypeField').getValue())&&(this.deviceId||this.groupId)&&(from=new Date(this.fromDate.getFullYear(),this.fromDate.getMonth(),this.fromDate.getDate(),this.fromTime.getHours(),this.fromTime.getMinutes(),this.fromTime.getSeconds(),this.fromTime.getMilliseconds()),to=new Date(this.toDate.getFullYear(),this.toDate.getMonth(),this.toDate.getDate(),
this.toTime.getHours(),this.toTime.getMinutes(),this.toTime.getSeconds(),this.toTime.getMilliseconds()),'showButton'===button.reference?('chart'===reportType?(button=this.getChart().getStore(),this.getChart().setSeries([])):button=this.getGrid().getStore(),button.showMarkers=this.showMarkers,button.load({params:{deviceId:this.deviceId,groupId:this.groupId,type:this.eventType,from:from.toISOString(),to:to.toISOString()}})):'exportButton'===button.reference&&(button=this.getGrid().getStore().getProxy().url,
this.downloadFile(button,{deviceId:this.deviceId,groupId:this.groupId,type:this.eventType,from:Ext.Date.format(from,'c'),to:Ext.Date.format(to,'c')})))},onClearClick:function(){var reportType=this.lookupReference('reportTypeField').getValue();this.clearReport(reportType)},clearReport:function(reportType){this.getGrid().getStore().removeAll();'trips'!==reportType&&'events'!==reportType&&'stops'!==reportType||Ext.getStore('ReportRoute').removeAll();'chart'===reportType&&this.getChart().getStore().removeAll()},
onSelectionChange:function(selection,selected){0<selected.length&&(selection=selected[0],this.fireEvent('selectreport',selection,!0),selection instanceof Traccar.model.ReportTrip&&this.selectTrip(selection),(selection instanceof Traccar.model.Event||selection instanceof Traccar.model.ReportStop)&&this.selectPositionRelated(selection))},selectDevice:function(device){device&&this.getGrid().getSelectionModel().deselectAll()},selectReport:function(object){var reportType=this.lookupReference('reportTypeField').getValue();
if(object instanceof Traccar.model.Position)if('route'===reportType)this.getGrid().getSelectionModel().select([object],!1,!0),this.getGrid().getView().focusRow(object);else if('events'===reportType||'stops'===reportType)object=this.getGrid().getStore().findRecord('positionId',object.get('id'),0,!1,!0,!0),this.getGrid().getSelectionModel().select([object],!1,!0),this.getGrid().getView().focusRow(object)},deselectFeature:function(){'trips'!==this.lookupReference('reportTypeField').getValue()&&this.getGrid().getSelectionModel().deselectAll()},
selectTrip:function(trip){var from,to;from=new Date(trip.get('startTime'));to=new Date(trip.get('endTime'));Ext.getStore('ReportRoute').removeAll();Ext.getStore('ReportRoute').showMarkers=this.showMarkers;Ext.getStore('ReportRoute').load({params:{deviceId:trip.get('deviceId'),from:from.toISOString(),to:to.toISOString()}})},selectPositionRelated:function(report){report.get('positionId')&&(report=Ext.getStore('ReportRoute').getById(report.get('positionId')))&&this.fireEvent('selectreport',report,!0)},
loadRelatedPositions:function(store,data){var reportObject,positionIds=[];Ext.getStore('ReportRoute').removeAll();for(store=0;store<data.length;store++)reportObject=data[store],reportObject.get('positionId')&&positionIds.push(reportObject.get('positionId'));0<positionIds.length&&Ext.getStore('Positions').load({params:{id:positionIds},scope:this,callback:function(records,operation,success){success&&(Ext.getStore('ReportRoute').showMarkers=this.showMarkers,Ext.getStore('ReportRoute').add(records),1===
records.length&&this.fireEvent('selectreport',records[0],!1))}})},loadRoute:function(store){var deviceIds,chartSeries,deviceStore;if('chart'===this.lookupReference('reportTypeField').getValue()){this.getChart().getAxes()[0].setTitle(Ext.getStore('ReportChartTypes').findRecord('key',this.chartType).get('name'));chartSeries=[];deviceIds=store.collect('deviceId');for(store=0;store<deviceIds.length;store++)deviceStore=Ext.create('Ext.data.ChainedStore',{source:'ReportRoute',filters:[{property:'deviceId',
value:deviceIds[store]}]}),chartSeries.push({type:'line',store:deviceStore,yField:this.chartType,xField:'fixTime',highlightCfg:{scaling:Traccar.Style.chartMarkerHighlightScaling},colors:[Traccar.app.getReportColor(deviceIds[store])],marker:{type:'circle',radius:Traccar.Style.chartMarkerRadius,fill:Traccar.app.getReportColor(deviceIds[store])}});this.getChart().setSeries(chartSeries)}},onChartMarkerClick:function(chart,item){this.fireEvent('selectreport',item.record,!0)},showSingleEvent:function(eventId){this.lookupReference('reportTypeField').setValue('events');
Ext.getStore('Events').load({id:eventId,scope:this,callback:function(records,operation,success){success&&(Ext.getStore('ReportEvents').add(records),0<records.length&&(records[0].get('positionId')||(Traccar.app.isMobile()?Traccar.app.showReports(!0):this.getView().expand()),this.getGrid().getSelectionModel().select([records[0]],!1,!0),this.getGrid().getView().focusRow(records[0])))}})},downloadFile:function(requestUrl,requestParams){Ext.Ajax.request({url:requestUrl,method:'GET',params:requestParams,
headers:{Accept:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'},binary:!0,success:function(response){var disposition,type,url,downloadUrl;disposition=response.getResponseHeader('Content-Disposition');disposition=disposition.slice(disposition.indexOf('\x3d')+1,disposition.length);type=response.getResponseHeader('Content-Type');response=new Blob([response.responseBytes],{type:type});'undefined'!==typeof window.navigator.msSaveBlob?window.navigator.msSaveBlob(response,disposition):
(url=window.URL||window.webkitURL,downloadUrl=url.createObjectURL(response),disposition&&Ext.dom.Helper.append(Ext.getBody(),{tag:'a',href:downloadUrl,download:disposition}).click(),setTimeout(function(){url.revokeObjectURL(downloadUrl)},100))}})},onTypeChange:function(combobox,newValue,oldValue){null!==oldValue&&this.clearReport(oldValue);'route'===newValue?(this.getGrid().reconfigure('ReportRoute',this.routeColumns),this.getView().getLayout().setActiveItem('grid')):'events'===newValue?(this.getGrid().reconfigure('ReportEvents',
this.eventsColumns),this.getView().getLayout().setActiveItem('grid')):'summary'===newValue?(this.getGrid().reconfigure('ReportSummary',this.summaryColumns),this.getView().getLayout().setActiveItem('grid')):'trips'===newValue?(this.getGrid().reconfigure('ReportTrips',this.tripsColumns),this.getView().getLayout().setActiveItem('grid')):'stops'===newValue?(this.getGrid().reconfigure('ReportStops',this.stopsColumns),this.getView().getLayout().setActiveItem('grid')):'chart'===newValue&&this.getView().getLayout().setActiveItem('chart');
this.updateButtons()},onGeocode:function(positionId){var position=Ext.getStore('ReportRoute').getById(positionId);position&&!position.get('address')&&Ext.Ajax.request({scope:this,method:'GET',url:'api/server/geocode',params:{latitude:position.get('latitude'),longitude:position.get('longitude')},success:function(response){position.set('address',response.responseText);position.commit();this.fireEvent('selectReport',position)},failure:function(response){Traccar.app.showError(response)}})},routeColumns:[{text:Strings.reportDeviceName,
...
{text:Strings.positionAltitude,dataIndex:'altitude',renderer:Traccar.AttributeFormatter.getFormatter('altitude')},{text:Strings.positionSpeed,dataIndex:'speed',renderer:Traccar.AttributeFormatter.getFormatter('speed')},{text:Strings.positionAddress,dataIndex:'address',renderer:function(value,metaData,record){return value?Traccar.AttributeFormatter.getFormatter('address')(value):'\x3ca href\x3d"#" onclick\x3d"Ext.fireEvent(\'routegeocode\', '+record.getId()+')" \x3e'+Strings.stateShowAddress+'\x3c/a\x3e'}}],
eventsColumns:[{text:Strings.positionFixTime,dataIndex:'serverTime',xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('serverTime')},{text:Strings.reportDeviceName,dataIndex:'deviceId',renderer:Traccar.AttributeFormatter.getFormatter('deviceId')},{text:Strings.sharedType,dataIndex:'type',renderer:function(value){return Traccar.app.getEventString(value)}},{text:Strings.sharedGeofence,dataIndex:'geofenceId',renderer:Traccar.AttributeFormatter.getFormatter('geofenceId')}],summaryColumns:[{text:Strings.reportDeviceName,
dataIndex:'deviceId',renderer:Traccar.AttributeFormatter.getFormatter('deviceId')},{text:Strings.sharedDistance,dataIndex:'distance',renderer:Traccar.AttributeFormatter.getFormatter('distance')},{text:Strings.reportAverageSpeed,dataIndex:'averageSpeed',renderer:Traccar.AttributeFormatter.getFormatter('speed')},{text:Strings.reportMaximumSpeed,dataIndex:'maxSpeed',renderer:Traccar.AttributeFormatter.getFormatter('speed')},{text:Strings.reportEngineHours,dataIndex:'engineHours',renderer:Traccar.AttributeFormatter.getFormatter('duration')},
{text:Strings.reportSpentFuel,dataIndex:'spentFuel',renderer:Traccar.AttributeFormatter.getFormatter('spentFuel')}],tripsColumns:[{text:Strings.reportDeviceName,dataIndex:'deviceId',renderer:Traccar.AttributeFormatter.getFormatter('deviceId')},{text:Strings.reportStartTime,dataIndex:'startTime',xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('startTime')},{text:Strings.reportStartAddress,dataIndex:'startAddress',renderer:Traccar.AttributeFormatter.getFormatter('address')},{text:Strings.reportEndTime,
dataIndex:'endTime',xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('endTime')},{text:Strings.reportEndAddress,dataIndex:'endAddress',renderer:Traccar.AttributeFormatter.getFormatter('address')},{text:Strings.sharedDistance,dataIndex:'distance',renderer:Traccar.AttributeFormatter.getFormatter('distance')},{text:Strings.reportAverageSpeed,dataIndex:'averageSpeed',renderer:Traccar.AttributeFormatter.getFormatter('speed')},{text:Strings.reportMaximumSpeed,dataIndex:'maxSpeed',renderer:Traccar.AttributeFormatter.getFormatter('speed')},
{text:Strings.reportDuration,dataIndex:'duration',renderer:Traccar.AttributeFormatter.getFormatter('duration')},{text:Strings.reportSpentFuel,dataIndex:'spentFuel',renderer:Traccar.AttributeFormatter.getFormatter('spentFuel')},{text:Strings.sharedDriver,dataIndex:'driverUniqueId',renderer:Traccar.AttributeFormatter.getFormatter('driverUniqueId')}],stopsColumns:[{text:Strings.reportDeviceName,dataIndex:'deviceId',renderer:Traccar.AttributeFormatter.getFormatter('deviceId')},{text:Strings.reportStartTime,
dataIndex:'startTime',xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('startTime')},{text:Strings.positionAddress,dataIndex:'address',renderer:Traccar.AttributeFormatter.getFormatter('address')},{text:Strings.reportEndTime,dataIndex:'endTime',xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('endTime')},{text:Strings.reportDuration,dataIndex:'duration',renderer:Traccar.AttributeFormatter.getFormatter('duration')},{text:Strings.reportEngineHours,dataIndex:'engineHours',
renderer:Traccar.AttributeFormatter.getFormatter('duration')},{text:Strings.reportSpentFuel,dataIndex:'spentFuel',renderer:Traccar.AttributeFormatter.getFormatter('spentFuel')}]});
...
Ext.define('Traccar.view.map.BaseMap',{extend:Ext.panel.Panel,xtype:'baseMapView',layout:'fit',getMap:function(){return this.map},getMapView:function(){return this.mapView},initMap:function(){var server,type,bingKey,zoom,maxZoom,target;server=Traccar.app.getServer();type=Traccar.app.getPreference('map',null);bingKey=server.get('bingKey');switch(type){case 'custom':server=new ol.layer.Tile({source:new ol.source.XYZ({url:server.get('mapUrl'),attributions:''})});break;case 'bingRoad':server=new ol.layer.Tile({source:new ol.source.BingMaps({key:bingKey,
...
center;zoom=this.getView().getMapView().getProjection();center=ol.proj.transform(this.getView().getMapView().getCenter(),zoom,'EPSG:4326');zoom=this.getView().getMapView().getZoom();this.fireEvent('mapstate',center[1],center[0],zoom)},updateGeofences:function(){this.getView().getGeofencesSource().clear();this.lookupReference('showGeofencesButton').pressed&&Ext.getStore('Geofences').each(function(geofence){var feature=new ol.Feature(Traccar.GeofenceConverter.wktToGeometry(this.getView().getMapView(),
geofence.get('area')));feature.setStyle(this.getAreaStyle(geofence.get('name'),geofence.get('attributes')?geofence.get('attributes').color:null));this.getView().getGeofencesSource().addFeature(feature);return!0},this)},zoomToAllDevices:function(){this.zoomToAllPositions(Ext.getStore('LatestPositions').getData().items)}});
...
name:'forceSettings',fieldLabel:Strings.serverForceSettings},{xtype:'combobox',name:'coordinateFormat',fieldLabel:Strings.settingsCoordinateFormat,store:'CoordinateFormats',displayField:'name',valueField:'key',editable:!1}]},{xtype:'fieldset',title:Strings.sharedPermissions,collapsible:!0,collapsed:!0,items:[{xtype:'checkboxfield',inputValue:!0,uncheckedValue:!1,name:'registration',fieldLabel:Strings.serverRegistration},{xtype:'checkboxfield',inputValue:!0,uncheckedValue:!1,name:'readonly',fieldLabel:Strings.serverReadonly},
...
columns:{defaults:{flex:1,minWidth:Traccar.Style.columnWidthNormal},items:[{text:Strings.sharedName,dataIndex:'name',filter:'string'},{text:Strings.userEmail,dataIndex:'email',filter:'string'},{text:Strings.userAdmin,dataIndex:'admin',renderer:Traccar.AttributeFormatter.getFormatter('admin'),filter:'boolean'},{text:Strings.serverReadonly,dataIndex:'readonly',hidden:!0,renderer:Traccar.AttributeFormatter.getFormatter('readonly'),filter:'boolean'},{text:Strings.userDeviceReadonly,dataIndex:'deviceReadonly',
renderer:Traccar.AttributeFormatter.getFormatter('deviceReadonly'),hidden:!0,filter:'boolean'},{text:Strings.userDisabled,dataIndex:'disabled',renderer:Traccar.AttributeFormatter.getFormatter('disabled'),filter:'boolean'},{text:Strings.userExpirationTime,dataIndex:'expirationTime',hidden:!0,renderer:Traccar.AttributeFormatter.getFormatter('expirationTime'),filter:'date'}]}});
Ext.define('Traccar.view.dialog.Group',{extend:Traccar.view.dialog.BaseEdit,title:Strings.groupDialog,items:{xtype:'form',items:[{xtype:'fieldset',title:Strings.sharedRequired,items:[{xtype:'textfield',name:'name',fieldLabel:Strings.sharedName,allowBlank:!1}]},{xtype:'fieldset',title:Strings.sharedExtra,collapsible:!0,collapsed:!0,items:[{xtype:'combobox',name:'groupId',fieldLabel:Strings.groupParent,store:'Groups',queryMode:'local',displayField:'name',valueField:'id',editable:!1}]}]}});
...
this.lookupReference('toolbarCommandsButton').setHidden(Traccar.app.getPreference('limitCommands',!1))},onGeofencesClick:function(){var admin,group;admin=Traccar.app.getUser().get('admin');group=this.getView().getSelectionModel().getSelection()[0];Ext.create('Traccar.view.BaseWindow',{title:Strings.sharedGeofences,items:{xtype:'linkGeofencesView',baseObjectName:'groupId',linkObjectName:'geofenceId',storeName:admin?'AllGeofences':'Geofences',baseObject:group.getId()}}).show()},onAttributesClick:function(){var admin,
group;admin=Traccar.app.getUser().get('admin');group=this.getView().getSelectionModel().getSelection()[0];Ext.create('Traccar.view.BaseWindow',{title:Strings.sharedComputedAttributes,items:{xtype:'linkComputedAttributesView',baseObjectName:'groupId',linkObjectName:'attributeId',storeName:admin?'AllComputedAttributes':'ComputedAttributes',baseObject:group.getId()}}).show()},onDriversClick:function(){var admin,group;admin=Traccar.app.getUser().get('admin');group=this.getView().getSelectionModel().getSelection()[0];
Ext.create('Traccar.view.BaseWindow',{title:Strings.sharedDrivers,items:{xtype:'linkDriversView',baseObjectName:'groupId',linkObjectName:'driverId',storeName:admin?'AllDrivers':'Drivers',baseObject:group.getId()}}).show()},onCommandsClick:function(){var admin,group;admin=Traccar.app.getUser().get('admin');group=this.getView().getSelectionModel().getSelection()[0];Ext.create('Traccar.view.BaseWindow',{title:Strings.sharedSavedCommands,items:{xtype:'linkSavedCommandsView',baseObjectName:'groupId',linkObjectName:'commandId',
storeName:admin?'AllCommands':'Commands',baseObject:group.getId()}}).show()},onNotificationsClick:function(){var admin,group;admin=Traccar.app.getUser().get('admin');group=this.getView().getSelectionModel().getSelection()[0];Ext.create('Traccar.view.BaseWindow',{title:Strings.sharedSavedCommands,items:{xtype:'linkNotificationsView',baseObjectName:'groupId',linkObjectName:'notificationId',storeName:admin?'AllNotifications':'Notifications',baseObject:group.getId()}}).show()},onSelectionChange:function(selection,
selected){var disabled=0===selected.length;this.lookupReference('toolbarGeofencesButton').setDisabled(disabled);this.lookupReference('toolbarAttributesButton').setDisabled(disabled);this.lookupReference('toolbarDriversButton').setDisabled(disabled);this.lookupReference('toolbarCommandsButton').setDisabled(disabled);this.lookupReference('toolbarNotificationsButton').setDisabled(disabled);this.callParent(arguments)}});
...
queryMode:'local',displayField:'name',valueField:'id',editable:!1,fieldLabel:Strings.sharedCalendar},{xtype:'hiddenfield',name:'area',allowBlank:!1,reference:'areaField'}]}]},buttons:[{text:Strings.sharedArea,glyph:'xf21d@FontAwesome',handler:'onAreaClick'},{text:Strings.sharedAttributes,handler:'showAttributesView'},{xtype:'tbfill'},{glyph:'xf00c@FontAwesome',tooltip:Strings.sharedSave,tooltipType:'title',minWidth:0,handler:'onSaveClick'},{glyph:'xf00d@FontAwesome',tooltip:Strings.sharedCancel,tooltipType:'title',
...
{text:Strings.notificationAlways,dataIndex:'always',renderer:Traccar.AttributeFormatter.getFormatter('always'),filter:'boolean'},{text:Strings.notificationWeb,dataIndex:'web',renderer:Traccar.AttributeFormatter.getFormatter('web'),filter:'boolean'},{text:Strings.notificationMail,dataIndex:'mail',renderer:Traccar.AttributeFormatter.getFormatter('mail'),filter:'boolean'},{text:Strings.notificationSms,dataIndex:'sms',renderer:Traccar.AttributeFormatter.getFormatter('sms'),filter:'boolean'}]}});
...
Ext.define('Traccar.view.dialog.DeviceDistanceController',{extend:Ext.app.ViewController,alias:'controller.deviceDistance',onSetClick:function(){var data={deviceId:this.getView().deviceId,totalDistance:this.lookupReference('totalDistance').getValue()};Ext.Ajax.request({scope:this,method:'PUT',url:'api/devices/'+data.deviceId+'/distance',jsonData:Ext.util.JSON.encode(data),callback:function(options,success,response){success||Traccar.app.showError(response)}});this.closeView()}});
...
text:Strings.sharedSavedCommands,glyph:'xf093@FontAwesome',handler:'onCommandsClick',reference:'settingsCommandsButton'},{text:Strings.loginLogout,glyph:'xf08b@FontAwesome',handler:'onLogoutClick'}]}});
...
attribute.get('valueType')&&chartTypesStore.add({key:'attribute.'+attribute.get('key'),name:attribute.get('name')})},calculateAttribute:function(data){data=data.attributes[this.attributeKey];return void 0!==data?Traccar.AttributeFormatter.getAttributeConverter(this.attributeKey)(data):data},onLaunch:function(){Ext.Ajax.request({scope:this,url:'api/server',callback:this.onServerReturn})},onServerReturn:function(options,success,response){options={};if(success){Traccar.app.setServer(Ext.decode(response.responseText));
...
onLogin:function(){this.login.close();this.loadApp()},loadApp:function(){var attribution;Ext.getStore('Groups').load();Ext.getStore('Drivers').load();Ext.getStore('Geofences').load();Ext.getStore('Calendars').load();Ext.getStore('ComputedAttributes').load();Ext.getStore('AllCommandTypes').load();Ext.getStore('Commands').load();Ext.getStore('AllNotificationTypes').load({callback:function(records,operation,success){operation=Ext.getStore('ReportEventTypes');success&&(operation.add({type:Traccar.store.ReportEventTypes.allEvents,
name:Strings.eventAll}),operation.loadData(records,!0))}});Ext.getStore('Notifications').load();Ext.getStore('ServerAttributes').loadData(Ext.getStore('CommonDeviceAttributes').getData().items,!0);Ext.getStore('ServerAttributes').loadData(Ext.getStore('CommonUserAttributes').getData().items,!0);Ext.getStore('UserAttributes').loadData(Ext.getStore('CommonUserAttributes').getData().items,!0);Ext.getStore('DeviceAttributes').loadData(Ext.getStore('CommonDeviceAttributes').getData().items,!0);Ext.getStore('GroupAttributes').loadData(Ext.getStore('CommonDeviceAttributes').getData().items,
!0);Ext.getStore('Devices').load({scope:this,callback:function(){this.asyncUpdate(!0)}});(attribution=Ext.get('attribution'))&&attribution.remove();Traccar.app.isMobile()?Ext.create('widget.mainMobile'):Ext.create('widget.main');if(attribution=Ext.Object.fromQueryString(window.location.search).eventId)this.fireEvent('showsingleevent',attribution),this.removeUrlParameter('eventId')},beep:function(){this.beepSound||(this.beepSound=new Audio('beep.wav'));this.beepSound.play()},soundPressed:function(){var soundButton=
Ext.getCmp('soundButton');return soundButton&&soundButton.pressed},removeUrlParameter:function(param){var params=Ext.Object.fromQueryString(window.location.search);delete params[param];Ext.Object.isEmpty(params)?window.history.pushState(null,null,window.location.pathname):window.history.pushState(null,null,window.location.pathname+'?'+Ext.Object.toQueryString(params))},asyncUpdate:function(first){var self=this,protocol,pathname;protocol='https:'===window.location.protocol?'wss:':'ws:';pathname=window.location.pathname.substring(0,
window.location.pathname.lastIndexOf('/')+1);protocol=new WebSocket(protocol+'//'+window.location.host+pathname+'api/socket');protocol.onclose=function(){Traccar.app.showToast(Strings.errorSocket,Strings.errorTitle);Ext.Ajax.request({url:'api/devices',success:function(response){self.updateDevices(Ext.decode(response.responseText))},failure:function(response){401===response.status&&window.location.reload()}});Ext.Ajax.request({url:'api/positions',headers:{Accept:'application/json'},success:function(response){self.updatePositions(Ext.decode(response.responseText))}});
